...
public enum ModuleDescriptionXML implements IXMLFileType {
  ...
  static final URI NAMESPACE_URI = URI.create(
      "http://www.optimizationBenchmarking.org/formats/evaluationModules/evaluationModules.1.0.xsd") //$NON-NLS-1$
      .normalize();
  ...
  public final URL getSchemaSource() throws IOException {
    ...
    u = this.getClass().getResource(//
        ModuleDescriptionXML.NAMESPACE.substring(
            ModuleDescriptionXML.NAMESPACE.lastIndexOf('/') + 1));
    if (u != null) {
      return u;
    }
