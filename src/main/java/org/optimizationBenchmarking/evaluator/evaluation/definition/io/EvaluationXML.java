...
public enum EvaluationXML implements IXMLFileType {
  ...
  static final URI NAMESPACE_URI = URI.create(
      "http://www.optimizationBenchmarking.org/formats/evaluationConfiguration/evaluationConfiguration.1.0.xsd") //$NON-NLS-1$
      .normalize();
  ...
  public final URL getSchemaSource() throws IOException {
    ...
    u = this.getClass().getResource(//
        EvaluationXML.NAMESPACE
            .substring(EvaluationXML.NAMESPACE.lastIndexOf('/') + 1));
    if (u != null) {
      return u;
    }
