package org.optimizationBenchmarking.evaluator.data.impl.flat;
...
public class FlatExperimentSetContext
  ...
  private static final int MODE_DIMENSION = (FlatExperimentSetContext.MODE_EXPERIMENT_SET
      + 1);
  /** we are in the instance */
  private static final int MODE_INSTANCE = (FlatExperimentSetContext.MODE_DIMENSION
      + 1);
  /** we are in the experiment */
  private static final int MODE_EXPERIMENT = (FlatExperimentSetContext.MODE_INSTANCE
      + 1);
  /** we are in the runs */
  private static final int MODE_RUNS = (FlatExperimentSetContext.MODE_EXPERIMENT
      + 1);
  /** we are in a run */
  private static final int MODE_RUN = (FlatExperimentSetContext.MODE_RUNS
      + 1);

  /** the mode names */
  private static final String[] MODE_NAMES = new String[FlatExperimentSetContext.MODE_RUN
      + 1];
   ...
   */
  public FlatExperimentSetContext(final IExperimentSetContext context) {
    super();

    if (context == null) {
      throw new IllegalArgumentException(//
          "IExperimentSetContext cannot be null."); //$NON-NLS-1$
    }
   ...
   */
  private final IDimensionContext __dimensionEnsure(
  ...
  public synchronized final void dimensionSetName(final String name) {
    final IDimensionContext context;
    ...
    try {
      ...
      throw new IllegalStateException(this.__errorLocation(//
          ((((((("Error while setting dimension parser class ") //$NON-NLS-1$
              + TextUtils.className(parserClass)) + " and lower bound ") //$NON-NLS-1$
              + lowerBound) + " and upper bound ") + upperBound) + '.'), //$NON-NLS-1$
          true), error);
    }
  }
  ...
  @Override
  @SuppressWarnings("resource")
  public synchronized final void dimensionSetType(
      final EDimensionType type) {
    final IDimensionContext context;
      ...
      case MODE_EXPERIMENT: {
        try {
          ...
          throw new IllegalStateException(this.__errorLocation(//
              ((((("Error while declaring parameter '") //$NON-NLS-1$
                  + name) + "' with description '") + desc) + //$NON-NLS-1$
                  "' to the experiment context."), //$NON-NLS-1$
              true), error);
        }
        return;
      }
      ...
      case MODE_RUNS: {
        try {
          ...
          throw new IllegalStateException(this.__errorLocation(//
              ((((("Error while declaring parameter '") //$NON-NLS-1$
                  + name) + "' with description '") + desc) + //$NON-NLS-1$
                  "' to the run set context."), //$NON-NLS-1$
              true), error);
        }
        return;
      }
   ...
   */
  private final IInstanceContext __instanceEnsure(final boolean forceNew) {
  ...
  public synchronized final void instanceSetName(final String name) {
    final IInstanceContext context;
  ...
  public synchronized final IInstanceSet getInstanceSet() {
  ...
  @Override
  @SuppressWarnings("incomplete-switch")
  public synchronized final IFeatureSet getFeatureSet() {
  ...
  private final IExperimentContext __experimentEnsure(
      final boolean forceNew) {
  ...
  public synchronized final void experimentSetName(final String name) {
    final IExperimentContext context;
  ...
  public final synchronized void experimentSetParameterValue(
    ...
    switch (this.m_mode) {
      ...
      case MODE_RUN: {
        try {
          this.m_run.setParameterValue(parameterName, parameterDescription,
              parameterValue, parameterValueDescription);
          return;
        } catch (final Throwable t) {
          error = t;
   ...
   */
  @SuppressWarnings("fallthrough")
  private final IInstanceRunsContext __runsEnsure(final boolean forceNew) {
  ...
  @Override
  @SuppressWarnings("resource")
  public synchronized final void runsSetInstance(final IInstance inst) {
    final IInstanceRunsContext context;
   ...
   */
  private final IRunContext __runEnsure(final boolean forceNew) {
    ...
    try {
      ...
      throw new IllegalStateException(this.__errorLocation(//
          ((("Error while adding data point numbers ") //$NON-NLS-1$
              + Arrays.toString(values)) + " to run."), //$NON-NLS-1$
          true), error);
    }
  }
  ...
  /** {@inheritDoc} */
  @Override
  public synchronized final IExperimentSet getExperimentSet() {
    final IExperimentSetContext main;
    IExperimentSet res;
