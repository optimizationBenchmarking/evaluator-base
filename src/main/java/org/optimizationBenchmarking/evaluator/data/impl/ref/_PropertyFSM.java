...
abstract class _PropertyFSM<VT extends _PropertyFSMRecord, CT, RT, OT>
  ...
  private static final int STATE_COMPILED = (_PropertyFSM.STATE_COMPILING
      + 1);
  /** the manager is currently finalizing the data */
  private static final int STATE_FINALIZING = (_PropertyFSM.STATE_COMPILED
      + 1);
  /** the finalization is finished */
  private static final int STATE_FINALIZED = (_PropertyFSM.STATE_FINALIZING
      + 1);
   ...
   */
  private static final String __checkPropertyName(
    ...
    "A property name cannot be null or empty, but '" //$NON-NLS-1$
        + n) + "' is '" //$NON-NLS-1$
    ) + propertyName) + "' was specified as property name."); //$NON-NLS-1$
  ...
  synchronized final void _setPropertyValue(final String propertyName,
    ...
    if ((_PropertyValueUnspecified.NAME.equalsIgnoreCase(n)) || //
      ...
      throw new IllegalArgumentException(
          ('\'' + n) + "' is not permitted as property name."); //$NON-NLS-1$
    }
    ...
    if (v instanceof String) {
      ...
      if (vn == null) {
        ...
        "The textual representation of a property value cannot be null or empty, but the value '"//$NON-NLS-1$
            + propertyValue) + "' of property '" //$NON-NLS-1$
