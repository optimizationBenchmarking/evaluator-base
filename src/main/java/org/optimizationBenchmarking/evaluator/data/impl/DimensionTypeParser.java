package org.optimizationBenchmarking.evaluator.data.impl;
...
public class DimensionTypeParser extends InstanceParser<EDimensionType> {
  ...
  static {
    ...
    EDimensionType.ITERATION_FE
        .ordinal()] = "objective function evaluation"; //$NON-NLS-1$
    DimensionTypeParser.HUMAN_READABLE[//
    EDimensionType.ITERATION_SUB_FE.ordinal()] = "sub-FE"; //$NON-NLS-1$
    DimensionTypeParser.HUMAN_READABLE[//
    EDimensionType.QUALITY_PROBLEM_DEPENDENT
        .ordinal()] = "quality measure (problem dependent)"; //$NON-NLS-1$
    DimensionTypeParser.HUMAN_READABLE[//
    EDimensionType.QUALITY_PROBLEM_INDEPENDENT
        .ordinal()] = "quality measure (problem independent)"; //$NON-NLS-1$
    DimensionTypeParser.HUMAN_READABLE[//
  ...
  public final EDimensionType parseString(final String string) {
    ...
    for (index = DimensionTypeParser.HUMAN_READABLE.length; (--index) >= 0;) {
      if (DimensionTypeParser.HUMAN_READABLE[index]
  ...
  public static final String getHumanReadable(final EDimensionType type) {
    if (type == null) {
      throw new IllegalArgumentException("Dimension type cannot be null."); //$NON-NLS-1$
    }
