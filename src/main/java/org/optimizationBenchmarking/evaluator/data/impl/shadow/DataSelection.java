...
public class DataSelection
    extends _MappedSelection<IExperimentSet, IExperiment, IInstanceRuns> {
   ...
   */
  public synchronized final void addExperiment(
   ...
   */
  public synchronized final void addInstanceRuns(
    ...
    outer: for (final IExperiment experiment : this.m_set.getData()) {
      for (final IInstanceRuns runs : experiment.getData()) {
        if (Compare.equals(instance, runs.getInstance())) {
          if (this._addSubElement(runs)) {
            this.m_parameters._addSubElements(//
                experiment.getParameterSetting());
            changed = true;
          }
          continue outer;
        }
  ...
  public final void addFeatureValue(final IFeature feature,
    ...
    if (Compare.equals(feature.getGeneralized().getValue(), value)) {
      throw new IllegalArgumentException(//
          "Cannot add generalized feature value."); //$NON-NLS-1$
    }

    for (final IInstance instance : this.m_set.getInstances().getData()) {
      if (Compare.equals(instance.getFeatureSetting().get(feature),
          value)) {
        this.addInstance(instance);
      }
    }
  ...
  public final void addParameterValue(final IParameter parameter,
    ...
    if (Compare.equals(parameter.getGeneralized().getValue(), value)) {
      throw new IllegalArgumentException(//
          "Cannot add generalized parameter value."); //$NON-NLS-1$
    }

    for (final IExperiment experiment : this.m_set.getData()) {
      if (Compare.equals(experiment.getParameterSetting().get(parameter),
          value)) {
        this.addExperiment(experiment);
      }
    }
