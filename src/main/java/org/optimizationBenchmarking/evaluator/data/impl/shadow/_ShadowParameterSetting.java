...
import org.optimizationBenchmarking.evaluator.data.impl.abstr.AbstractParameterSetting;
...
final class _ShadowParameterSetting extends AbstractParameterSetting {
  ...
  public final boolean containsValue(final Object value) {
    for (final ShadowParameterValue entry : this.m_data) {
      if (Compare.equals(entry, value) || //
          Compare.equals(entry.getValue(), value)) {
        return true;
      }
    }
    return false;
   ...
   * @return the value
   */
  private final ShadowParameterValue __getParameterValue(
      final Object key) {
    ...
    if (key instanceof IParameter) {
      ...
      for (final ShadowParameterValue entry : this.m_data) {
        ...
        if (Compare.equals(compare, useKey)) {
          return entry;
        }
      }
    }
