...
abstract class _PropertyFSMPropertiesBuilder<PVT extends PropertyValue<?>, PT extends Property<?>, PST extends _PropertySet<?, ?, ?>>
   ...
   */
  private static final String __mergeNames(final String a,
      final String b) {
    final int len1, len2;
  ...
  private final PT __mapProperty(final _PropertyFSMPropertyRecord rec,
    ...
    makePv: {
      ...
      parseA: {
        ...
        try {
          for (final Object value : values) {
            propertyValue = this.m_owner
                ._normalize(parser.parseObject(value));
      ...
      isClass: {
        for (final Object value : values) {
          ...
          if (!(TextUtils
              .couldBeClassName(stringValue.substring(i + 1, len - 1)))) {
            break isClass;
          }
          propertyValue = this.m_owner
              ._normalizeLocal(stringValue.substring(0, i));
          if (propertyValue == null) {
            break isClass;
          }
      ...
      for (final Object value : values) {
        ...
        if (propertyValue instanceof String) {
          ...
          stringValue = this.m_owner
              ._normalize(String.valueOf(propertyValue));
        }
        q = this._createPropertyValue(this.m_owner._normalize(stringValue),
            this.m_owner._normalize(descriptions.get(value)),
            this.m_owner._normalize(propertyValue));
    ...
    propertyValues = parsed.values()
        .toArray(new PropertyValue[parsed.size()]);
    ...
    return this._createProperty(n,
        this.m_owner._normalize(_PropertyFSMPropertiesBuilder
            .__normalizeDesc(n, rec.m_propertyDesc)),
        type, propertyValues, (refCount < totalCount));
  }
